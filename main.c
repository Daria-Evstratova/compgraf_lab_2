...
// Добавим новую переменную для начального фона
GLuint startBgTexture = 0;  // Текстура начального фона

// Добавим глобальные переменные для отслеживания состояния кнопок
...
GLuint LoadTexture(const char *filename)
{
    ...
    if (!image) {
        ...
        return 0; // Если текстура не загрузилась, возвращаем 0
    }
...
void ShowPlayer()
{
    ...
    glBegin(GL_QUADS);
        glTexCoord2f(0.0f, 0.0f); glVertex2f(playerPosX, playerPosY);
        glTexCoord2f(1.0f, 0.0f); glVertex2f(playerPosX + frameWidth, playerPosY);
        glTexCoord2f(1.0f, 1.0f); glVertex2f(playerPosX + frameWidth, playerPosY + frameHeight);
        glTexCoord2f(0.0f, 1.0f); glVertex2f(playerPosX, playerPosY + frameHeight);
    glEnd();
